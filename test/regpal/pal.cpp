...
namespace
{
    ...
    void* LoadSymbol(char const* symbolName)
    {
        ...
        auto getDispenser = GetSymbol(mod, "MetaDataGetDispenser");
        if (getDispenser == nullptr)
        {
            std::cerr << "Failed to find " << symbolName << " in module: " << coreClrPath << std::endl;
            return nullptr;
        }
    ...
    MetaDataGetDispenser GetDispenser = (MetaDataGetDispenser)LoadSymbol("MetaDataGetDispenser");
}
...
void* pal::GetBaselineSymbol(char const* symbolName)
{
    return ::LoadSymbol(symbolName);
}